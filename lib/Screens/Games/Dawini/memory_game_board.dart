...
class _MemoryGameBoardState extends State<MemoryGameBoard> {
    ...
    gameStream.listen((MemoryGame? game) {
      if (game != null) {
        setState(() {
          ...
          _gameTime = getGameplayTime(userLevel, userDifficultyLevel);
                ...
                Column(
                  ...
                  children: [
                    ...
                    SizedBox(
                        ...
                        child: RoundedContainer(text: _isGameCompleted ? '0s' :'${_gameTime}s', backgroundColor: CustomColors.redPrimary, strokeColor: CustomColors.darkBlue, txtColor: Colors.white, icon: Icons.timer,)
                    ),
                    const SizedBox(height: 20,),
                    Container(
                      margin: const EdgeInsets.symmetric(horizontal: 20),
                          ...
                          const Duration(milliseconds: 160),
                            ...
                            setState(() async {
                              if(!_isFinished){
                                ...
                                updatedScore=calculateScore(userDifficultyLevel,userLevel,userScore);
  ...
  int calculateScore(String difficulty, int level, int oldScore) {
    int baseScore;
    ...
    return oldScore + baseScore + (level - 1) * 100;
  }
  ...
  List<dynamic> moveToNextLevel() {
    ...
    int nextLevel;
    debugPrint(GameDifficulty.DIFFICILE.displayName);
    debugPrint(GameDifficulty.MOYEN.displayName);
    debugPrint(GameDifficulty.FACILE.displayName);
